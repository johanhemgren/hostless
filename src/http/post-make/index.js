const{HOST}=require("@architect/shared/constants"),{encrypt,setPassword}=require("@architect/shared/encryption"),{compress}=require("@architect/shared/compression"),getPostedData=t=>{const o=Buffer.from(t,"base64").toString("ascii");return JSON.parse(o)},escapeHtml=t=>t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");exports.handler=async function(o){const{title:a,hostUrl:s,bodyHtml:c,preferedPassword:r}=getPostedData(o.body),i=r!==""?r:null,e=s.replace(/(^\w+:|^)\/\//,""),d=compress({title:a,bodyHtml:c},e),n=setPassword(i),l=await encrypt(d,e,n),p=`http://${e}.${HOST}/ess/${encodeURIComponent(l)}`;return{headers:{"content-type":"application/json; charset=utf8","cache-control":"no-cache, no-store, must-revalidate, max-age=0, s-maxage=0"},statusCode:200,body:JSON.stringify({html:`
      <p class="mb-3">Add this as a TXT-record to your domains DNS:</p>
      <code class="inline-block p-4 mb-1 rounded border border-indigo-600 bg-indigo-100 ">"hostl=${escapeHtml(n)}"</code>
